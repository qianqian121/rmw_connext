  ...
  const char * typesupport)
{
  ...
  if (using_introspection_c_typesupport(typesupport)) {
    CREATE_TYPE_CODE(INTROSPECTION_CPP_TYPE)
  } else if (using_introspection_cpp_typesupport(typesupport)) {
    CREATE_TYPE_CODE(INTROSPECTION_C_TYPE)
  } else {
    RMW_SET_ERROR_MSG("Unexpected typesupport identifier received");
    goto fail;
  }
...
#define SET_C_STRING_VALUE(TYPE, METHOD_NAME) \
  { \
    if (member->is_array_) { \
      ...
      auto typed_ros_message = static_cast<const TYPE *>(ros_message); \
      if (!typed_ros_message) { \
        RMW_SET_ERROR_MSG("Failed to cast ROS message as TYPE"); \
        return false; \
      } \
      const TYPE * value = reinterpret_cast<const TYPE *>( \
        typed_ros_message + member->offset_); \
      DDS_ReturnCode_t status = dynamic_data->METHOD_NAME( \
        NULL, \
        static_cast<DDS_DynamicDataMemberId>(i + 1), \
        value->data); \
...
#define SET_CPP_STRING_VALUE(TYPE, METHOD_NAME) \
  { \
    if (member->is_array_) { \
      DDS_DynamicData dynamic_data_member(NULL, DDS_DYNAMIC_DATA_PROPERTY_DEFAULT); \
      DDS_ReturnCode_t status = dynamic_data->bind_complex_member( \
...
#define SET_SUBMESSAGE_VALUE(dynamic_data, i, INTROSPECTION_TYPE) \
  ...
  auto sub_members = static_cast<const INTROSPECTION_TYPE(MessageMembers) *>( \
    member->members_->data); \
  if (!sub_members) { \
    RMW_SET_ERROR_MSG("sub members handle is null"); \
    return false; \
  } \
  bool published = _publish(&sub_dynamic_data, sub_ros_message, sub_members, typesupport); \
  if (!published) { \
  ...
  const void * untyped_members, const char * typesupport)
{
  ...
  if (using_introspection_c_typesupport(typesupport)) {
    ...
    PUBLISH(INTROSPECTION_CPP_TYPE)
  } else {
    RMW_SET_ERROR_MSG("Invalid typesupport handle")
    return false;
  }
...
#define GET_CPP_STRING_VALUE(TYPE, METHOD_NAME) \
  { \
    if (member->is_array_) { \
      ARRAY_SIZE() \
      if (array_size > (std::numeric_limits<DDS_DynamicDataMemberId>::max)()) { \
...
#define GET_C_STRING_VALUE(TYPE, METHOD_NAME) \
  { \
    if (member->is_array_) { \
      ARRAY_SIZE() \
      if (array_size > (std::numeric_limits<DDS_DynamicDataMemberId>::max)()) { \
...
#define GET_SUBMESSAGE_VALUE(dynamic_data, i, INTROSPECTION_TYPE) \
  ...
  bool success = _take(&sub_dynamic_data, sub_ros_message, sub_members, typesupport); \
  status = dynamic_data->unbind_complex_member(sub_dynamic_data); \
  if (!success) { \
    return false; \
  } \
